	...
	if (ep & USB_EP_IN){
		if (USB_ep_ready(ep) && pipe_can_read(pipe) > 2){
			...
			if (bank == 0){ // Helps the optimzer constant-fold, since the multiply is expensive
				USB_ep_start_bank(ep, 0, pipe_read_ptr(pipe), pipe->size);
			}else{
				USB_ep_start_bank(ep, 1, pipe_read_ptr(pipe), pipe->size);
			}
		...
		if (USB_ep_ready(ep) && pipe_can_write(pipe) > 2){
			...
			if (bank == 0){
				USB_ep_start_bank(ep, 0, pipe_write_ptr(pipe), pipe->size);
			}else{
				USB_ep_start_bank(ep, 1, pipe_write_ptr(pipe), pipe->size);
			}
